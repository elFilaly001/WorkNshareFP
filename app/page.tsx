...
export default function PuzzleEntrance() {
  ...
  const handlePieceClick = (pieceId: number) => {
    if (selectedPieces.length === 0) {
      ...
      // Swap pieces
      const firstPieceIndex = shuffledPieces.findIndex(
        (p) => p.id === selectedPieces[0],
      );
      const secondPieceIndex = shuffledPieces.findIndex(
        (p) => p.id === pieceId,
      );

      const newPieces = [...shuffledPieces];
      [newPieces[firstPieceIndex], newPieces[secondPieceIndex]] = [
        newPieces[secondPieceIndex],
        newPieces[firstPieceIndex],
      ];
                  ...
                  <div className="grid grid-cols-5 md:grid-cols-6 gap-2 md:gap-3 mb-8">
                    {shuffledPieces.map((piece, index) => (
                      <motion.button
                        ...
                        className={`
                        bg-[#2261A6]/30 backdrop-blur-md border border-[#2261A6]/40
